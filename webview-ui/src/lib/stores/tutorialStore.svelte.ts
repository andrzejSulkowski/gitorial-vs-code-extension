...
export const tutorialStore = {
  get tutorial() { return tutorialState.tutorial; },
  get currentStep() { return tutorialState.currentStep; },
  get isLoading() { return tutorialState.isLoading; },
  get error() { return tutorialState.error; },
  ...
  handleMessage(message: ExtensionToWebviewTutorialMessage) {
    ...
    switch (message.type) {
      ...
      case 'step-changed':
        if (!tutorialState.tutorial) return;
